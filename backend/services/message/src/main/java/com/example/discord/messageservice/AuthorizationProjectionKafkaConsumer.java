...
final class AuthorizationProjectionKafkaConsumer {
    ...
    AuthorizationProjectionKafkaConsumer(
        ...
        this.consumer = new AuthorizationProjectionConsumer(store, AuthorizationAudience.MESSAGE);
        this.deadLetterKafka = deadLetterKafka;
        this.deadLetterTopic = (topicPrefix == null || topicPrefix.isBlank() ? "discord" : topicPrefix.trim())
            + ".authz.message.v1.dead-letter";
    }

    @KafkaListener(
        topics = "${discord.kafka.topic-prefix:discord}.authz.message.v1",
        groupId = "authz-projection-message"
    )
