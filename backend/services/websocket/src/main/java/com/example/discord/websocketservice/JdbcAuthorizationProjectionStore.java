...
public class JdbcAuthorizationProjectionStore implements AuthorizationProjectionStore {
    private static final String CONSUMER = "websocket-service";
    ...
    public boolean apply(AuthzProjectionUpdated event) {
        if (jdbc.update("INSERT INTO consumer_inbox(consumer_name,event_id,processed_at) VALUES (?,?,?) ON CONFLICT DO NOTHING",
            CONSUMER, event.eventId(), Timestamp.from(Instant.now())) == 0) return false;
        jdbc.update("""
