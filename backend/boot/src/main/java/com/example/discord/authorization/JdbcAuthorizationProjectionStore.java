...
@Repository
@Profile("postgres")
class JdbcAuthorizationProjectionStore implements AuthorizationProjectionStore {
    private static final String CONSUMER = "boot-authorization";
    private final JdbcTemplate jdbc;

    JdbcAuthorizationProjectionStore(JdbcTemplate jdbc) { this.jdbc = jdbc; }

    @Override
    @Transactional
    public boolean apply(AuthzProjectionUpdated event) {
        if (inbox(event.eventId()) == 0) return false;
        jdbc.update("""
    ...
    private int inbox(UUID eventId) {
        return jdbc.update("INSERT INTO consumer_inbox(consumer_name,event_id,processed_at) VALUES (?,?,?) ON CONFLICT DO NOTHING",
            CONSUMER, eventId, Timestamp.from(Instant.now()));
    }
