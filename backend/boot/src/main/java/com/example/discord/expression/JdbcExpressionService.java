...
final class JdbcExpressionService implements ExpressionService {
    ...
    public CustomEmoji createCustomEmoji(UUID guildId, String name, String imageObjectKey, UUID creatorId) {
    ...
    public List<CustomEmoji> customEmojis(UUID guildId) {
    ...
    public void deleteCustomEmoji(UUID guildId, UUID emojiId) {
        uuid(guildId, "guildId"); uuid(emojiId, "emojiId");
        try (Connection c = dataSource.getConnection(); PreparedStatement s = c.prepareStatement("DELETE FROM custom_emojis WHERE id = ? AND guild_id = ?")) {
            s.setObject(1, emojiId); s.setObject(2, guildId);
            if (s.executeUpdate() == 0) throw new ExpressionNotFoundException("custom emoji not found");
        } catch (SQLException e) { throw new IllegalStateException("failed to delete custom emoji", e); }
    }
    public Sticker createSticker(UUID guildId, String name, String description, UUID creatorId) {
    ...
    public List<Sticker> stickers(UUID guildId) {
    ...
    public void addReaction(UUID channelId, UUID messageId, String emojiKey, UUID userId) {
    ...
    public void removeReaction(UUID channelId, UUID messageId, String emojiKey, UUID userId) {
        key(channelId, messageId, emojiKey); uuid(userId, "userId");
    ...
    public List<ReactionSummary> reactionSummaries(UUID channelId, UUID messageId) {
