...
| T120 | Agent Harness PDCA Loop | P0 | generated by qa/new-ticket.ps1 |
| T190 | Re-enable Tracked CI Operations Gates | P0 | CI recovery follow-up: qa workflow jobs must only reference tracked scripts and their source/test dependencies |
| T57 | Process Tree Cleanup Helper | P0 | generated by qa/new-ticket.ps1 |

| T191 | Kafka Gateway Confidentiality Boundary Decision | P0 | generated by qa/new-ticket.ps1 |

## Runtime Split Freeze

2026-07-13 runtime split Plan 00이 다음 후속 확장을 동결한다. Plan 01이 완료될 때까지 administrator console/dashboard, generic RBAC/ABAC, global role/permission management, Kafka hardening, media 기능은 새 작업으로 시작하지 않는다. 자세한 controller/configuration 소유권은 `docs/03-tasking/runtime-split-ownership-map.md`를 기준으로 한다. 현재 진행 중인 독립 작업은 수정하거나 폐기하지 않는다.
## Source Clusters
