...
foreach($r in $rows){foreach($n in @('count','error_count','error_rate','p50_ms','p95_ms','p99_ms','max_ms')){[double]$v=0;if(-not [double]::TryParse($r.$n,[Globalization.NumberStyles]::Float,[Globalization.CultureInfo]::InvariantCulture,[ref]$v) -or [double]::IsNaN($v) -or [double]::IsInfinity($v) -or $v -lt 0){throw "invalid numeric field $n"}};$count=[double]$r.count;$errors=[double]$r.error_count;$expectedRate=if($count -eq 0){0}else{$errors/$count};if($errors -gt $count -or [math]::Abs(([double]$r.error_rate)-$expectedRate) -gt 0.000001 -or [double]$r.p50_ms -gt [double]$r.p95_ms -or [double]$r.p95_ms -gt [double]$r.p99_ms -or [double]$r.p99_ms -gt [double]$r.max_ms){$decision='REJECT';$reasons+="invalid metric ordering:$($r.variant)/$($r.operation)"};if([double]$r.error_rate -ge .01 -or [double]$r.p99_ms -ge 500){$decision='REJECT';$reasons+="threshold:$($r.variant)/$($r.operation)"}}
