...
결과는 `qa/artifacts/c4-chat-scale/<run-id>/` 아래의 `run.json`, `latency.tsv`, `db-stats.tsv`, `replica-lag.tsv`, `plans/`에만 기록된다. `run.json`에는 variant, seed, duration, UTC, git SHA만 저장하며 password·DSN·token·raw body는 저장하지 않는다. 실행 종료 시 compose는 `down -v --remove-orphans`로 정리된다.

Replica replay pause drill은 자동 실행하지 않는다. 필요 시 수동으로 `docker compose -f qa/c4-chat-scale/docker-compose.yml exec -T replica psql -U c4_user -d c4chat -c "SELECT pg_wal_replay_pause();"`를 실행하고, 재개는 `SELECT pg_wal_replay_resume();`를 사용한다.
